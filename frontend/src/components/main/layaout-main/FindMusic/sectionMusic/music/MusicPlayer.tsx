...
export const MusicPlayer: React.FC<MusicPlayerProps> = ({ title, artist, genre, audioUrl }) => {
    ...
    return (
        <div className="text-2xl w-full text-tertiary flex justify-between items-center border-t-2 gap-6 lg:gap-10 h-20 bg-primary">
            ...
            <p>-</p>
            <p className=' w-40 md:w-32 lg:w-36 justify-center  xl:w-72 2xl:w-80 flex text-xs md:text-lg lg:text-lg xl:text-xl 2xl:text-2xl  mr-5'>{artist}</p>
            <p className='hidden xl:flex'>-</p>
            <p className=' w-28 md:w-32 lg:w-36 justify-center  xl:w-72 2xl:w-80 xl:flex text-xs md:text-lg lg:text-lg xl:text-xl  2xl:text-2xl  hidden mr-5'>{genre[0]}</p>
        </div>
    );
};
